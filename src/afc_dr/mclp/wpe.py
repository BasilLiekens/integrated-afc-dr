...
def _step_wpe_inner(
    ...
    x_hat_e = stftBuff_e[:, : p.D, :]
    x_hat_l = y_l - jnp.conj(p.G.transpose(0, 2, 1)) @ (y_tilde_l + y_tilde_e)
    ...
    lmbd = jnp.mean(
        jnp.abs(stftBuff_e[:, : p.D, :] + stftBuff_l[:, : p.D, :]) ** 2,
        axis=1,
        keepdims=True,
    )  # XXX: return back to PSD normalization?
...
def run_wpe(
    ...
    p_final, output = jax.lax.scan(_inner_loop, init=p, xs=(x_e, x_l))
